...
The algorithm is unchanged --- replay buffer, target network, $\epsilon$-greedy, the Bellman target
above --- but the \emph{representation} and \emph{scale} change completely.

\subsection{How it works}
A single observation is no longer an index but an \textbf{image}. The standard Atari pipeline turns
each frame to greyscale, resizes it to $84\times84$, skips $4$ frames per action, and
\textbf{stacks the last $4$ frames} into a $(4,84,84)$ tensor --- the stack is what lets the agent
perceive motion (a single frame cannot show which way the ball is travelling). A
\textbf{convolutional} Q-network (three conv layers, then a $512$-unit dense layer, then one
Q-value per action) reads that stack. The rest is the same recipe scaled up: a $100$k
\texttt{uint8} image replay buffer ($\sim$6\,GB), rewards clipped to $\{-1,0,+1\}$, a frame-based
loop over ${\sim}1$--$2$M frames with the target network synced every $10$k frames, and
checkpointing so a disconnected GPU session can resume.
